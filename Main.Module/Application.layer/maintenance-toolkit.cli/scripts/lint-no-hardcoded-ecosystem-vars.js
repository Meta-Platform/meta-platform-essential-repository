...
// voltar a hardcodar uma das 19 variáveis de ecosystem-defaults.json.
...
for (const rel of files) {
    ...
    const hits = Object.keys(obj).filter(k => VARS19.has(k))
    if (hits.length) { offenders++; total += hits.length; console.error(`[HARDCODE] ${rel}\n           ${hits.join(", ")}`) }
}
if (offenders) { console.error(`\nFALHA: ${offenders} arquivo(s), ${total} literal(is) das 19 vars do ecossistema. Remova-os (herança via pkg-exec).`); process.exit(1) }
console.log(`OK: ${files.length} startup-params.json verificados, 0 hardcodes das 19 vars do ecossistema.`)
