...
const _PackageNameOf = (metadata) => {
    const namespace = metadata && metadata.package && metadata.package.namespace
    return namespace ? String(namespace).replace("@/", "") : undefined
}

const _ResolveSocket = (resource, { packageName, installDataDirPath, socketsDirName, supervisorSocketsDirName }) => {
...
const _ResolveStorage = (resource, { packageName, installDataDirPath, storageDirName }) => {
...
const ResolvePackageResourceParams = ({
    ...
    const _Resolve = (params, Resolver) =>
        Object
            .keys(params || {})
