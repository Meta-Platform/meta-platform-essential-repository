...
const _ExpandHome = (value: unknown): unknown =>
    typeof value === "string" && value.startsWith("~")
        ? value.replace("~", os.homedir())
        : value

const _SamePath = (literal: unknown, resolved: string): boolean => {
    if(typeof literal !== "string" || !literal) return false
    return resolve(_ExpandHome(literal) as string) === resolve(resolved)
}
...
const DescribeCollision = ({ packageNamespace, packagePath, parameter, kind, owner, currentValue, resourceValue }: any): string =>
...
const BuildCollisionError = (collisions: ResourceCollision[]): Error =>
    new Error(
        `resource-params-handler: ${collisions.length} recurso(s) declarado(s) colidem com parâmetros que já têm valor.\n` +
        collisions.map((collision: ResourceCollision) => `  - ${DescribeCollision(collision)}`).join("\n")
    )
