...
const _BuildAgentLinkRules = (boundParams: any) => {
    const namespaceList = boundParams ? ExtractNamespaceListByBoundParams(boundParams) : []
    return namespaceList.map((namespace: string) => ({
...
const _ResolveContentPackage = ({ dependency, metadataHierarchy }: { dependency: string, metadataHierarchy: MetadataHierarchy }) => {
    if(dependency){
